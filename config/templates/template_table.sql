...
alter table change_me 
add constraint change_me_pk 
primary key (change_me_id);

alter table change_me 
add constraint change_me_uk1 
unique(change_me_code);

alter table change_me 
add constraint change_me_ck1 
check(change_me_code = trim(upper(change_me_code)));

alter table change_me 
add constraint change_me_ck2 
check(change_me_seq = trunc(change_me_seq));

alter table change_me 
add constraint change_me_fk1 
foreign key (changeme) 
references changeme_dest(changeme);

create index change_me_idx1 
on change_me(changeme);